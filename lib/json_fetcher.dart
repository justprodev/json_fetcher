...
abstract class HttpJsonFetcher<T> {
  ...
  Stream<T> fetch(String url, {allowErrorWhenCacheExists: false, nocache: false}) => HttpClient._fetch(url, allowErrorWhenCacheExists: allowErrorWhenCacheExists, nocache: nocache).asyncMap((s)=>_parse(url, s));
...
class HttpClient {
    ...
    JsonCacheManager().readAsString(url, headers: _headers, nocache: nocache).listen((String s) { controller.add(s); hasData = true; },
        onError: (e) {
          ...
          if (e is HttpException && e.message.contains('401') &&
              _onBearerExpire != null) _onBearerExpire();
  ...
  static Future<http.Response> put(String url, String json, {Map<String,String> headers}) async {
    ...
    h.addAll(_headers);
    if(headers!=null) h.addAll(headers);
    var response = await _client.put(url, body: json, headers: h);
    final String contentType = response.headers[HttpHeaders.contentTypeHeader] ?? "application/json";
    if(!contentType.contains("charset")) response.headers[HttpHeaders.contentTypeHeader] = contentType + ";charset=utf-8";
    if(response.statusCode == 401) { if(_onBearerExpire!=null) _onBearerExpire(); throw "$url: Пользователь не авторизован"; }
    return response;
  ...
  static Future<http.Response> get(String url) async {
    Map<String, String> h = {"Content-Type": "application/json"};
    h.addAll(_headers);
    var response = await _client.get(url, headers: h);
    final String contentType = response.headers[HttpHeaders.contentTypeHeader] ?? "application/json";
    if(!contentType.contains("charset")) response.headers[HttpHeaders.contentTypeHeader] = contentType + ";charset=utf-8";
    if (response.statusCode < 200 || response.statusCode >= 400) {
      if(response.statusCode == 401) { if(_onBearerExpire!=null) _onBearerExpire(); throw "$url: Пользователь не авторизован"; }
  ...
  static void logout() => _onBearerExpire();
...
class JsonCacheManager {
  ...
  Stream<String> readAsString(String url, {Map<String, String> headers, nocache: false}) {
    StreamController<String> controller = _downloads[url];

    if(controller!=null && !controller.isClosed) controller.close(); // prev download started, drop it
